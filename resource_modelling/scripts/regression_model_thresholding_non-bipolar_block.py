...
def models(res_class):
    ...
    X_train_hls, X_test_hls, Y_train_hls, Y_test_hls = model_selection.train_test_split(X_hls, Y_hls, test_size = 0.3, shuffle=True, random_state=0)
    X_train_estimate, X_test_estimate, Y_train_estimate, Y_test_estimate= model_selection.train_test_split(X_estimate, Y_estimate, test_size = 0.3, shuffle=True, random_state=0)
    
    #Feature Scaling - Normalization
    #scaler = MinMaxScaler().fit(X_unseen_ts_synth)#X_train
    #X_train = scaler.transform(X_train)
    #X_test = scaler.transform(X_test)

    #Feature Scaling - Normalization
    #scaler = MinMaxScaler().fit(X_unseen_ts_synth) #DD should I use X_train??
    ...
    #Feature Scaling - Standardization
    #scaler = StandardScaler().fit(X_unseen_ts_synth) #DD should I use X_train??
    X_unseen_ts_synth = scaler.transform(X_unseen_ts_synth)
    X_unseen_ts_hls = scaler.transform(X_unseen_ts_hls)
    X_unseen_ts_estimate = scaler.transform(X_unseen_ts_estimate)
    ...
    gscv = GridSearchCV(
        ...
        param_grid={
            ...
            'epsilon': [0.1, 1, 2, 5, 20, 10, 50, 100, 200, 250, 500, 1000, 5000],
        ...
        cv=5, n_jobs = -1, refit = True, verbose = 2)
...
def generate_graph_test_set(parameter, res_class):
    ...
    fig.savefig('../graphs/%s/plot_%s_vs_%s_thresholding_non-bipolar_block.png' % (directory_name, 'pe', 'LUT'), bbox_inches='tight')
...
#remove timing 
#remove ram_style, Res from
columns_to_remove = ['ram_style', 'Resources from:', 'FPGA', 'finn_commit', 'vivado_version', 'vivado_build_no', 'TargetClockPeriod', 'EstimatedClockPeriod', 'Delay', 'TargetClockFrequency [MHz]', 'EstimatedClockFrequency [MHz]']
